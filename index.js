...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5511942115778@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'gtts':
				 if (!isNsfw) return reply('❗𝐒𝐨𝐦𝐞𝐧𝐭𝐞 𝐩𝐯❗')
					if (args.length < 1) return client.sendMessage(from, '❗𝐂𝐚𝐝𝐞̂ 𝐨 𝐭𝐞̂𝐱𝐭𝐨?❗', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, '❗𝐂𝐚𝐝𝐞̂ 𝐨 𝐭𝐞̂𝐱𝐭𝐨?❗', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'salada':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/a/ECDzsJH.jpg`)
				...
				case 'tisu':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/a/ZLiohsT.jpg`)
