...
𝗘𝘀𝗽𝗲𝗿𝗼 𝗾𝘂𝗲 𝗴𝗼𝘀𝘁𝗲 𝗱𝗼 𝗴𝗿𝘂𝗽𝗼♥️`
				...
				teks = `➵𝗧𝗮 𝘃𝗲𝗻𝗱𝗼 𝗲𝘀𝘀𝗲 𝗰𝗮𝗿𝗮 𝗮𝗾𝘂𝗶? @${num.split('@')[0]} 𝗘𝗹𝗲 𝗳𝗼𝗶 𝗺𝗮𝗺𝗮𝗿 𝗲𝗺 𝗼𝘂𝘁𝗿𝗼 𝗹𝘂𝗴𝗮𝗿😔🔥 `
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
		 	...
		 	case 'mute':
				...
				case 'yumeko':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/SSyK5cS.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '𝗬𝗨𝗠𝗘𝗞𝗢✓\n➵𝗔𝗱𝗺 𝗱𝗮 𝗳𝗰𝗰 ᬊ͜͡𝑹𝑨ϟ𝑶℘⚡\n𝗖𝗮𝗻𝗮𝗹:https://youtube.com/channel/UC6ntk5-tZ5m-bVHTGH0cEiw\n𝗜𝗻𝘀𝘁𝗮: off\nWa.me/+12605779962'})
					break
				case 'mia':
					memein = await kagApi.memeindo()
