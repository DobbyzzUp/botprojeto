	...
	client.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/a/FimIixN.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '𝐂𝐑𝐈𝐀𝐃𝐎𝐑: 𝐷𝑂𝐵𝐵𝑌♱᭄\n𝐂𝐎𝐍𝐓𝐀𝐓𝐎 wa.me/+5511911219615\n𝐈𝐍𝐒𝐓𝐀: to sem aff kk\n\n\nDígite /bot para ver os comandos basicos para criar um bot})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
        ...
        case 'promover':
				...
				case 'Banir':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Banido⚡ : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'admins':
