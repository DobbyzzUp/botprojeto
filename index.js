...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '❗Aguarde...(caso nao for, tente novamente)❗',
				success: '✔️ Completo ✔️',
				error: {
					stick: '❗Erro ao transformar imagem em figurinha❗',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
					...
					admin: '❗Comando exclusivo pra admins, seu membro comum 🤬❗',
					Badmin: '❗É necessário o bot ser um admin!❗'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5519997908058@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'stiker':
				case 'sticker':
     if (!isNsfw) return reply('⚡disponivel apenas no pv⚡')
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await client.downloadAndSaveMediaMessage(encmedia)
						ran = getRandom('.webp')
					...
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, '⚡Cadê o texto?⚡', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('❗Texto muito grande, igual meu p❗')
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '🌹𝐃𝐎𝐍𝐎: 𝐷𝑂𝐵𝐵𝑌♱᭄\n🌹𝐈𝐍𝐒𝐓𝐀:off\n🌹𝐂𝐎𝐍𝐓𝐀𝐓𝐎:wa.me/5511911219615\n\n\nDìgite /bot para ver comandos basicos para criar um bot'})
					break
				case 'kirito':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/VuICDTD.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '🍁Adm kirito indo banir membro comum👺🍁'})
					break
				case 'tisu':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/hmjQDL3.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Tisu del Zapϟ\nDono e fundador da fcc ᬊ͜͡𝑹𝑨ϟ𝑶⚡\n➣Canal no YT:https://youtube.com/channel/UC7JdGFqRraNbKLCv5UTWtVA\n➣Insta:https://instagram.com/tisuzz?igshid=175cmdn23fk6c'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Olha isso mano 🤤'})
					break
				case 'bot':
			     	memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/dPUVFF6.png`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '🔗Comandos basicos para bot no Termux🔗:\n\n🔗$termux-setup-storage\n🔗$pkg upgrade && pkg update\n🔗$pkg install git\n🔗$pkg install wget\n🔗$pkg install libwebp\n🔗$pkg install ffmpeg\n🔗$pkg install nodejs\n🔗$git clone (git que deseja clonar)\n🔗$cd (nome do repositório)\n🔗$bash install.sh\n🔗$(npm start) ou (node index.js) para gerar seu qr code e iniciar seu bot\n\n\n➣By 𝐷𝑂𝐵𝐵𝑌♱᭄ hehe 👻'})
					break
				case 'regras':
					reply('🔰REGRAS DO GRUPO🔰:\n\n➣• Sem travas❌\n➣• Sem flood❌\n➣• Sem Wa.me e números❌\n➣• Sem desrespeito❌\n➣• Sem divulgação ou links❌\n➣• Sem conteúdos +18, gore e cp❌\n➣•Membro das rcts ou qualquer grupo de ataques sem tag❌\n➣• Não ir no pv dos adms pra pedir travas❌\n\n⚡Evite descumprir as regras para nao ser banido <3⚡')
		 		break
				case 'belle3':
					memein = await kagApi.memeindo()
				...
				case 'prefixo':
					...
					prefix = args[0]
					reply(`⚡𝐷𝑂𝐵𝐵𝑌♱᭄,O prefixo foi alterado com sucesso para : ${prefix}⚡`)
					break
				case 'loli':
				...
				case 'limparchat':
					...
					reply('⚡𝐷𝑂𝐵𝐵𝑌♱᭄ os chats do seu bot foram limpos com sucesso⚡')
					break
				case 'tag':
        ...
        case 'promover':
					...
					if (mentioned.length > 1) {
					 ...
					 for (let _ of mentioned) {
						...
						mentions(`o @${mentioned[0].split('@')[0]} Foi promovido pra admin, Respeitem ok?😂⚡`, mentioned, true)
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`O @${mentioned[0].split('@')[0]} agora é um mebro comum KKK⚡`, mentioned, true)
				...
				case 'banir':
					...
					if (mentioned.length > 1) {
						...
						mentions(`O @${mentioned[0].split('@')[0]} foi banido 🤭`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'admins':
     ...
     if (!isNsfw) return reply('⚡disponivel apenas no pv⚡')
				...
				case 'bemvindo':
					...
					if (Number(args[0]) === 1) {
						...
						reply('⚡Mensagens de boas vindas ativadas nesse grupo⚡')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('⚡Mensagens de boas vindas desativadas nesse grupo⚡')
					} else {
